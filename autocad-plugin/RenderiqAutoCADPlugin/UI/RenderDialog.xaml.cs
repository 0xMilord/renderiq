...
namespace RenderiqAutoCADPlugin.UI
{
    public partial class RenderDialog : Window
    {
        ...
        public RenderDialog(Document doc, string accessToken)
        {
            InitializeComponent();
            _doc = doc;
        ...
        private void LoadPreview()
        {
            try
            {
                ...
                byte[] previewData = ViewExporter.ExportViewToImage(_doc, 512);
        ...
        private async void RenderButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var settings = new RenderSettings
                {
                    ...
                    Style = (StyleComboBox.SelectedItem as System.Windows.Controls.ComboBoxItem)?.Content?.ToString()?.ToLower() ?? "photorealistic",
                    AspectRatio = (AspectRatioComboBox.SelectedItem as System.Windows.Controls.ComboBoxItem)?.Content?.ToString() ?? "16:9"
                };

                // Export view
                ProgressText.Text = "Exporting view to image...";
                byte[] imageData = ViewExporter.ExportViewWithSettings(_doc, settings);
