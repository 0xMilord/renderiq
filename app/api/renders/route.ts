        ...
        if (chainRenders.length > 0) {
          ...
          if (mostRecentRender) {
            ...
            if (mostRecentRender.outputUrl) {
              try {
                console.log('📥 Fetching reference render image:', mostRecentRender.outputUrl);
                ...
                console.log('✅ Reference render image fetched, size:', referenceRenderImageData.length);
              } catch (error) {
                console.error('❌ Failed to fetch reference render image:', error);
              }
            }
          } else {
        ...
        try {
          ...
          if (referenceRender && referenceRender.status === 'completed') {
            ...
            if (referenceRender.outputUrl) {
              try {
                console.log('📥 Fetching reference render image:', referenceRender.outputUrl);
    ...
    try {
      ...
      if (type === 'video') {
        ...
        if (referenceRenderPrompt && referenceRenderImageData) {
          ...
          console.log('🔗 Using contextual prompt with reference render:', contextualPrompt.substring(0, 100));
        }
