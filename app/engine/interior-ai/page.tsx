...
export default function InteriorAIPage() {
  ...
  const handleCreateChain = async (projectId: string) => {
    ...
    try {
      ...
      const chainName = project ? `${project.name} - Interior Chain` : 'New Interior Chain';
      
      const result = await createRenderChain(
        projectId,
        chainName,
        'Interior AI render chain'
      );

      if (result.success && result.data) {
        router.push(`/engine/interior-ai/${result.data.id}`);
      } else {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-[2400px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-foreground mb-2">Interior AI Engine</h1>
          <p className="text-lg text-muted-foreground">
            Transform your interior designs with AI-powered rendering
          </p>
          <p className="text-sm text-muted-foreground mt-2">
            Select a project to start generating interior renders
          </p>
        </div>
        ...
        {projects.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {projects.map((project) => (
              <ProjectCard
                ...
                // Add the chain creation functionality
                onContinue={() => handleCreateChain(project.id)}
                isCreating={creating === project.id}
              />
            ))}
          </div>
        ) : (
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground mb-6">
              Create your first project to start generating interior AI renders
            </p>
            <Button
